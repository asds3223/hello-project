...
static Choice searched_choice(Searcher& searcher, Position& p, std::mt19937& rng,
    ...
    if(topkProb<=0.0 || topk<=1 || p.ply()<topkMinPly || p.ply()>topkMaxPly) return out;
    if(std::generate_canonical<double,32>(rng) >= topkProb) return out;

    auto lm=legal(p);
    if(lm.size()<2) return out;
    sort_static(p,lm);
    ...
    for(Move m:candidates) scored.push_back({m,probe_move(searcher,p,m,probeMs,probeDepth)});
    std::stable_sort(scored.begin(),scored.end(),[](const P&a,const P&b){return a.s>b.s;});
    if(static_cast<int>(scored.size())>topk) scored.resize(static_cast<size_t>(topk));
    const int bestProbe=scored.front().s;
    std::vector<P> near;
    for(const auto& q:scored) if(q.s>=bestProbe-margin) near.push_back(q);
...
int main(int argc,char**argv){
    ...
    std::string outPath="data/selfplay_00000.bin", auxPath;
    std::string nnuePath;
    ...
    if(std::ifstream(outPath,std::ios::binary).good() || std::ifstream(auxPath,std::ios::binary).good()){
        std::cerr<<"refusing to overwrite existing output\n"; return 2;
    }
    std::ofstream out(outPath,std::ios::binary|std::ios::trunc), aux(auxPath,std::ios::binary|std::ios::trunc);
    if(!out || !aux){std::cerr<<"cannot open output/aux\n";return 2;}
    ...
    auto workerFn=[&](int workerId){
        ...
        while(true){
            ...
            for(int turn=p.ply();turn<225 && !game_won(p);turn++){
                ...
                Move played, teacherBest; int teacherScore=0; bool randomized=false;
                ...
                if(!searched){
                    ...
                    auto c=searched_choice(searcher,p,rng,depth,movetime,topkRandomProb,topk,topkMargin,
                                           topkProbeCandidates,topkProbeMs,topkProbeDepth,topkMinPly,topkMaxPly);
